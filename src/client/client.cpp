...
void connection()
{
	...
	while (1)
	{
		...
		if (buff.find(':') == std::string::npos && buff.find(',') != std::string::npos)
		{
			...
			if (a[0][0] == '1')
			{
				struct address addr = start_msg_sv(atoi(a[1].c_str()));
				address2.sin_port = addr.port;
				address2.sin_addr.s_addr = inet_addr(addr.ip.c_str());
				close(sv);
				sv = socket(AF_INET, SOCK_STREAM, 0);
				connect(sv, (sockaddr *)&address2, sizeof(address2));
			...
			{
				address2.sin_port = atoi(a[2].c_str());
				struct in_addr ip_addr;
				address2.sin_addr.s_addr = inet_addr(a[1].c_str());
				std::this_thread::sleep_for(std::chrono::milliseconds(100));
				close(sv);
				sv = socket(AF_INET, SOCK_STREAM, 0);
				if (connect(sv, (sockaddr *)&address2, sizeof(address2)) < 0)
				{
